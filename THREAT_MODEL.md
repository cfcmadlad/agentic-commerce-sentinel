...
One page, per layer: what it stops, what it explicitly does not, and why. This is a defense-only system throughout ([README](README.md#security-safety-and-threat-model)). Nothing here generates, launches, or automates an attack against a real system; the attack generators exist solely to produce synthetic test traffic against this project's own detectors, and every automated finding escalates to a human rather than acting unilaterally.
...
**Does not stop:** `unauthorized_subdelegation`, a hand-off that is genuinely, validly signed at every link, and every link individually stays within its parent's declared authority, but the human who granted the *root* mandate never actually authorized this particular downstream agent to hold it at all. Containment checks authority *width* against a parent, not agent-identity continuity across the whole chain. Measured at 2.59% recall on this specific sub-variant, a disclosed, undone gap, not a rounding error ([README](README.md#the-held-out-evaluation-a-case-study-in-disclosed-failure)).

**Architectural disclosure, stated plainly:** this check is real production code (`containment/engine.py`, `containment/gate.py`), exercised the same way in the headline evaluation as every other layer, but it is not called from `service.main.decide()`, the live API's one automatic per-session decision path -- only Layers 1, 2, and 3 are. It is reachable today through a separate, explicit call (`GET /mandates/{id}/chain`, and the `/delegation` demo view), so a transaction can be allowed by `decide()` while its own delegation chain would fail containment if checked. This was a deliberate choice made with the submission deadline immediate: wiring a new call into the one live decision path this close to the deadline, without the time to re-verify it doesn't regress anything Layers 1-3 already guarantee (including the Z3-proved properties above, none of which currently encode this call boundary), was judged riskier than shipping the gap named honestly. Closing it for real is the natural next step, not a same-session patch.
...
**Does not stop, and cannot ever be relied on alone to stop:** anything by itself. It only ever adds a block on top of what Layers 1-2 already allow, never overrides a deterministic rejection ([README](README.md#security-safety-and-threat-model)). Trained entirely on synthetic session timing; the sensitivity grid found that widening scripted-client pacing to sit inside legitimate jitter halves recall on exactly the two variants this layer exists to catch, at which point it no longer significantly beats the rules-only baseline ([README](README.md#generator-parameter-sensitivity), [EXCEPTIONS.md](EXCEPTIONS.md)). This is the layer most in need of retraining before any real-world use; real agent traffic will not match this generator's timing distribution.
...
**Real-world distribution shift.** Every number in this document comes from a synthetic generator ([README](README.md#security-safety-and-threat-model)). The rules layers transfer directly to real data since they are explicit logic, but Layer 3 and the collusion layer are both trained/calibrated on synthetic timing and volume distributions that real agent traffic is not guaranteed to match.
