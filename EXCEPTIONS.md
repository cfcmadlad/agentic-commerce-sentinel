...
definitions in `docs/adr/0003-held-out-class-evaluation.md`.
...
plausibly be nudged to catch without moving a great many legitimate sessions with it (the cost
sweep below shows the threshold basin is broad, but not free to move).

**What would resolve it.** Nothing free. Lowering the threshold trades this exact miss for more
false positives elsewhere, at the rate the cost sweep below already reports. This is disclosed as the
...
**What it is.** The calibration reliability diagram this evaluation produces is dominated by a single
near-zero bin holding 3,752 of the test residual's rows. Every bin between roughly 0.1 and 0.9 holds
fewer than ten sessions each.
...
The cost sweep this evaluation produces reports roughly 21.6 wrongly blocked legitimate sessions per
10,000, at the threshold this project actually runs at.
